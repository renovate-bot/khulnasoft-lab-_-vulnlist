...
Before you can contribute to Vulnlist, you need to configure your development environment.
...
You should fork the Vulnlist repo using the "Fork" button at the top right of the Vulnlist GitHub [site](https://github.com/khulnasoft-lab/vulnlist/). You will be doing your development in your fork, then submit a pull request to Vulnlist. There are many resources how to use GitHub effectively, we will not cover those here.

## Commit guidelines

In the Vulnlist project we like commits and pull requests (PR) to be easy to understand and review. Open source thrives best when everything happening is over documented and small enough to be understood.
...
If you made it this far and all the tests are passing, it's time to submit a Pull Request (PR) for Vulnlist. Submitting a PR is always a scary moment as what happens next can be an unknown. The Vulnlist project strives to be easy to work with, we appreciate all contributions. Nobody is going to yell at you or try to make you feel bad. We love contributions and know how scary that first PR can be.
...
And lastly, when proposed changes are modifying user-facing functionality or output, it is expected the PR will include updates to the documentation as well. Vulnlist is not a project that is heavy on documentation. This will mostly be updating the README and help for the tool.
