...
from vulnstick.cli.config import (
    ResultSet,
    ScanMatrix,
    Tool,
)
from vulnstick.cli.config import Application as VulnstickApplication
...
class Application(VulnstickApplication, DataClassDictMixin):
    pass
...
class Config(DataClassDictMixin):
    vulnstick: Vulnstick = field(default_factory=Vulnstick)
    ...
    def vulnstick_application_config(self, test_configurations: list[Test]) -> Application:
        images = []
        for test in test_configurations:
            images += test.images
        return Application(
            default_max_year=self.vulnstick.default_max_year,
            result_sets={
                "pr_vs_latest_via_sbom": ResultSet(
                    description="latest vulnerability data vs current vulnlist data with latest grype tooling (via SBOM ingestion)",
                    matrix=ScanMatrix(
                        images=images,
                        tools=self.vulnstick.tools,
                    ),
                ),
            },
        )
    ...
    def provider_data_source(self, providers: list[str]) -> tuple[list[str], list[str], Application]:
        ...
        return cached_providers, uncached_providers, self.vulnstick_application_config(tests)
...
@click.group(help="Manage vulnstick configuration that drives the quality gate testing")
@click.pass_context
...
def show_config(cfg: Config):
    ...
    def enum_asdict_factory(data: list[tuple[str, Any]]) -> dict[Any, Any]:
        ...
        #       request_timeout: 125
        #       runtime:
        #       existing_input: !!python/object/apply:vulnlist.provider.InputStatePolicy
        #           - keep
        #       existing_results: !!python/object/apply:vulnlist.provider.ResultStatePolicy
        #           - delete-before-write
        #       on_error:
        #           action: !!python/object/apply:vulnlist.provider.OnErrorAction
        #           - fail
        #           input: !!python/object/apply:vulnlist.provider.InputStatePolicy
        #           - keep
        #           results: !!python/object/apply:vulnlist.provider.ResultStatePolicy
        #           - keep
        #           retry_count: 3
        #           retry_delay: 5
        #       result_store: !!python/object/apply:vulnlist.result.StoreStrategy
        #           - flat-file
        #
...
def write_vulnstick_config(cfg: Application, path: str = ".vulnstick.yaml"):
    logging.info(f"writing vulnstick config to {path!r}")
...
def vulnstick_version_changed():
    logging.info("determining whether vulnstick version changed")
    ...
    for line in changes:
        ...
        if 'git = "https://github.com/khulnasoft-lab/vulnstick"' in line:
            return True
...
def select_providers(cfg: Config, output_json: bool):
    ...
    if not selected_providers:
        ...
        for test in cfg.tests:
            ...
            search_globs = [f"src/vulnlist/providers/{test.provider}/**"]

            for additional_provider in test.additional_providers:
                search_globs.append(f"src/vulnlist/providers/{additional_provider.name}/**")
...
def validate_test_tool_versions(cfg: Config):
    ...
    for idx, tool in enumerate(cfg.vulnstick.tools):
        if tool.name != "grype":
            continue
...
@cli.command(name="configure", help="setup vulnstick and grype-db configurations for building a DB")
@click.argument("provider_names", metavar="PROVIDER", nargs=-1, required=True)
@click.pass_obj
def configure(cfg: Config, provider_names: list[str]):
    logging.info(f"preparing vulnstick and grype-db configurations with {provider_names!r}")

    cached_providers, uncached_providers, vulnstick_app_cfg = cfg.provider_data_source(provider_names)
    ...
    write_vulnstick_config(vulnstick_app_cfg)
...
def build_db(cfg: Config):
    ...
    for provider in state.uncached_providers:
        ...
        subprocess.run(["vulnlist", "-v", "run", provider], check=True)
