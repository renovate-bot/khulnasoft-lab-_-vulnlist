...
To capture raw tool output and store into the local `.vulnstick` directory for
further analysis:
```
make capture
```
...
1. **Comparing vulnerability results taken at different times is invalid**.
   We leverage the vulnstick result-set feature to capture all vulnerability
...
One way of working is to simply run `vulnstick` and `gate.py` in the `test/quality` directory.
...
Using the SHA above, we can run `vulnstick` to see which results are available:
```shell
$ vulnstick result list --result-set pr_vs_latest_via_sbom | grep 808f6cf3cf4473eb39ff9bb47ead639d2ed71255b75b9b140162b58c6102bcc9
...
```shell
vulnstick label explore 5bf0611b-183f-4525-a1ab-f268f62f48b6
```
...
After iterating on all the changes we need using `vulnstick label explore`, we're now ready to commit changes. Since
...
Both this project and `vulnstick` require Python 3.10.
...
Now you should be able to run both `vulnstick` and `./gate.py`.

## Troubleshooting

As noted above, vulnstick requires Python 3.10. If you try to run with an older version, such as
...
Traceback (most recent call last):
  File "./vulnerability-match-labels/sboms.py", line 12, in <module>
    import vulnstick
  File "/grype/test/quality/vulnerability-match-labels/venv/lib/python3.8/site-packages/vulnstick/__init__.py", line 4, in <module>
    from . import arrange, artifact, capture, cli, comparison, label, store, tool, utils
  File "/grype/test/quality/vulnerability-match-labels/venv/lib/python3.8/site-packages/vulnstick/arrange.py", line 4, in <module>
    from vulnstick import artifact
  File "/grype/test/quality/vulnerability-match-labels/venv/lib/python3.8/site-packages/vulnstick/artifact.py", line 482, in <module>
    class ResultSet:
  File "/grype/test/quality/vulnerability-match-labels/venv/lib/python3.8/site-packages/vulnstick/artifact.py", line 484, in ResultSet
