...
from vulnstick import artifact, comparison, store, utils
from vulnstick.cli import config, display
from configure import Config, read_config_state

# see the .vulnstick.yaml configuration for details
default_result_set = "pr_vs_latest_via_sbom"
vulnstick.utils.grype_db.raise_on_failure(False)
...
def get_namespaces_from_db() -> list[str]:
    ...
    # TODO: this is hardcoded, but should be configurable or key off of vulnstick config
...
def validate_image(
    ...
    relative_comparison = vulnstick.compare_results(
    ...
    results, label_entries, comparisons_by_result_id, stats_by_image_tool_pair = vulnstick.compare_results_against_labels(
        descriptions=descriptions,
        year_max_limit=cfg.default_max_year,
        label_entries=label_entries,
        matches_filter=matches_filter,
...
def main(images: list[str], always_run_label_comparison: bool, breakdown_by_ecosystem: bool, verbosity: int, result_set: str):
    ...
    for result_set in result_sets:
        ...
        if breakdown_by_ecosystem:
            ...
            results_by_image, label_entries, stats = vulnstick.compare_results_against_labels_by_ecosystem(
