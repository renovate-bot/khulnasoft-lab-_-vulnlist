...
# reside in a different directory than where the vulnlist repo is, then you will need to
# set the `GRYPE_PATH` and/or `GRYPE_DB_PATH` environment variables for the development
# shell to function. You can add these to a local .env file in the vulnlist repo root.

# clone the vulnlist repo
git clone git@github.com:khulnasoft-lab/vulnlist.git
cd vulnlist
...
# Any provider from the output of "vulnlist list" is valid.
# Specify multiple as a space-delimited list:
# make dev providers="oracle wolfi nvd"
$ make dev provider="oracle"

Entering vulnlist development shell...
• Configuring with providers: oracle ...
• Writing grype config: /Users/wagoodman/code/vulnlist/.grype.yaml ...
• Writing grype-db config: /Users/wagoodman/code/vulnlist/.grype-db.yaml ...
• Activating poetry virtual env: /Users/wagoodman/Library/Caches/pypoetry/virtualenvs/vulnlist-slR_vCr2-py3.9 ...
• Installing editable version of vulnlist ...
• Building grype ...
• Building grype-db ...
...
$ grype db status
Location:  /Users/wagoodman/code/vulnlist/.cache/grype/5  # <--- this is the local DB we just built
...

# also note that we're using a development build of grype
$ which grype
/Users/wagoodman/code/vulnlist/bin/grype
```

The development builds of grype and grype-db provided are derived from `../grype` and `../grype-db` paths relative to the vulnlist project.
If you want to use a different path, you can set the `GRYPE_PATH` and `GRYPE_DB_PATH` environment variables. This can be
persisted by adding a `.env` file to the root of the vulnlist project:

```bash
# example .env file in the root of the vulnlist repo
...
If you want to use a locally-editable copy of vulnlist while you develop without the custom development shell:

```bash
poetry shell
pip uninstall vulnlist  #... if you already have vulnlist installed in this virtual env
pip install -e .
```
...
Vulnlist is a CLI tool that downloads and processes vulnerability data from various sources (in the codebase, these are called "providers").

<!-- repo path: docs/vulnlist-run-workflow.drawio -->
<!-- asset comment: https://github.com/khulnasoft-lab/vulnlist/issues/102#issuecomment-1456403838 -->
<img src="https://user-images.githubusercontent.com/590471/223163266-e73d2595-f320-4607-a016-f1b22aad45c7.svg" width="600" />

Conceptually, one or more invocations of Vulnlist will produce a single data directory which Grype-DB uses to create a Grype database:

<!-- repo path: docs/vulnlist+grype-db-workflow.drawio -->
<!-- asset comment: https://github.com/khulnasoft-lab/vulnlist/issues/102#issuecomment-1456408327 -->
<img src="https://user-images.githubusercontent.com/590471/223167464-aca39d4b-699a-47da-b852-fea904ba9824.svg" width="600" />

Additionally, the Vulnlist CLI tool is optimized to run
...
orchestrating multiple Vulnlist calls to prepare the input data:

<!-- repo path: docs/grype-db-actions.drawio -->
<!-- asset comment: https://github.com/khulnasoft-lab/vulnlist/issues/102#issuecomment-1456415533 -->
<img src="https://user-images.githubusercontent.com/590471/223165191-8b06b696-f7b5-4a92-912a-c7110c1cd324.svg" width="600" />

For more information about how Grype-DB uses Vulnlist see [the Grype-DB documentation](https://github.com/anchore/grype-db/blob/main/DEVELOPING.md#architecture).


### Vulnlist Providers

A "Provider" is the core abstraction for Vulnlist and represents a single source of vulnerability data. Vulnlist is a CLI wrapper
around multiple vulnerability data providers.

All provider implementations should...
- live under `src/vulnlist/providers` in their own directory (e.g. the NVD provider code is under `src/vulnlist/providers/nvd/...`)
- have a class that implements the [`Provider` interface](https://github.com/khulnasoft-lab/vulnlist/blob/1285a3be0f24fd6472c1f469dd327541ff1fc01e/src/vulnlist/provider.py#L73)
- be centrally registered with a unique name under [`src/vulnlist/providers/__init__.py`](https://github.com/khulnasoft-lab/vulnlist/blob/1285a3be0f24fd6472c1f469dd327541ff1fc01e/src/vulnlist/providers/__init__.py)
...
Each provider has a "workspace" directory within the "vulnlist root" directory (defaults to `./data`) named after the provider.

```yaml
data/                       # the "vulnlist root" directory
...
```json
{
    ...
    "schema": {
        "version": "1.0.0",
        "url": "https://raw.githubusercontent.com/khulnasoft-lab/vulnlist/main/schema/provider-workspace-state/schema-1.0.0.json"
    }
}
```
Where:
...
```json
{
    "schema": "https://raw.githubusercontent.com/khulnasoft-lab/vulnlist/main/schema/vulnerability/os/schema-1.0.0.json",
...
Possible vulnerability schemas supported within the vulnlist repo are:
- [Generic OS Vulnerability](https://github.com/khulnasoft-lab/vulnlist/tree/main/schema/vulnerability/os)
- [GitHub Security Advisories](https://github.com/khulnasoft-lab/vulnlist/tree/main/schema/vulnerability/github-security-advisory)
- [NVD Vulnerability](https://github.com/khulnasoft-lab/vulnlist/tree/main/schema/vulnerability/nvd)
...
Each provider has a configuration object defined next to the provider class. This object is used in the vulnlist application
configuration and is passed as input to the provider class. Take the debian provider configuration for example:

```python
from dataclasses import dataclass, field

from vulnlist import provider, result
...
The `runtime` field is used to configure common behaviors of the provider that are enforced within the `vulnlist.provider.Provider` subclass. Options include:
...
To add a new provider, you will need to create a new provider class  under `/src/vulnlist/providers/<name>` that inherits from `provider.Provider` and implements:
...
All results must conform to a [particular schema](https://github.com/khulnasoft-lab/vulnlist/tree/main/schema), today there are a few kinds:
...
- add a new entry under the dispatch table in `src/vulnlist/providers/__init__.py` mapping your provider name to the class
- add the provider configuration to the application configuration under `src/vulnlist/cli/config.py` (specifically the `Providers` dataclass)
...
#### **1. Fork Vulnlist and add the new provider.**

Take a look at the example provider in the `example` directory. You are encouraged to copy `example/awesome/*` into
`src/vulnlist/providers/YOURPROVIDERNAME/` and modify it to fit the needs of your new provider, however, this is not required:

```bash
# from the root of the vulnlist repo
cp -a example/awesome src/vulnlist/providers/YOURPROVIDERNAME
...
Once implemented, you should be able to see the new provider in the `vulnlist list` command and run it with `vulnlist run <name>`.
...
_**At this point you can optionally open a Vulnlist PR with your new provider and a Maintainer can help with the next steps.**_ Or if you'd like to get PR changes merged faster you can continue with the next steps.
...
#### **3. In Vulnlist: add a new test case to `tests/quality/config.yaml` for the new provider.**
...
#### **4. In Vulnlist: swap the tools to your Grype branch in `tests/quality/config.yaml`.**
...
`vulnstick.tools[*]` entries for grype to use the a version that points to your fork (w.g. `your-fork-username/grype@main`).
If you don't have any grype changes needed then you can skip this step.


#### **5. In Vulnlist: add new "vulnerability match labels" to annotate True and False positive findings with Grype.**
...
[Vulnstick](github.com/khulnasoft-lab/vulnstick) to do this:
...
# list your results
$ vulnstick result list | grep grype
...
$ vulnstick label explore 75d1fe75-0890-4d89-a497-b1050826d9f6

# You can use the vulnstick TUI to label results:
...
#### **6. In Vulnlist: run the quality gate.**
...
Vulnlist uses the labels in the vulnerability-Match-Labels repo via a git submodule. We've already added labels locally
...
_Note: you will not be able to open a Vulnlist PR that passes PR checks until the labels are merged into the vulnerability-match-labels repo._

Once the PR is merged in the vulnerability-match-labels repo you can update the submodule in Vulnlist to point to the latest commit in the vulnerability-match-labels repo.
...
#### **8. In Vulnlist: open a PR with your new provider.**

The PR will also run all of the same quality gate checks that you ran locally.

If you have Grype changes, you should also create a PR for that as well. The Vulnlist PR will not pass PR checks until the Grype PR is merged and the `test/quality/config.yaml` file is updated to point back to the `latest` Grype version.
...
1. You will need to add the new schema to the Vulnlist repo in the `schemas` directory.
2. Grype-DB will need to be updated to support the new schema in the `pkg/provider/unmarshal` and `pkg/process/v*` directories.
3. The Vulnlist `tests/quality/config.yaml` file will need to be updated to use development `grype-db.version`, pointing to your fork.
4. The final Vulnlist PR will not be able to be merged until the Grype-DB PR is merged and the `tests/quality/config.yaml` file is updated to point back to the `latest` Grype-DB version.


## What might need refactoring?

Looking to help out with improving the code quality of Vulnlist, but not sure where to start?

The best way is to look for [issues with the `refactor` label](https://github.com/khulnasoft-lab/vulnlist/issues?q=is%3Aissue+is%3Aopen+label%3Arefactor).

More general ways would be to use `radon` to search for complexity and maintainability issues:

```
$ radon cc src --total-average -nb
src/vulnlist/provider.py
    M 115:4 Provider._on_error - B
src/vulnlist/providers/alpine/parser.py
    M 73:4 Parser._download - C
    M 178:4 Parser._normalize - C
    M 141:4 Parser._load - B
    C 44:0 Parser - B
src/vulnlist/providers/amazon/parser.py
...
...
$ wily rank
-----------Rank for Maintainability Index for bdb4983 by KhulnaSoft Security on 2022-12-25.------------
╒═════════════════════════════════════════════════╤═════════════════════════╕
│ File                                            │   Maintainability Index │
╞═════════════════════════════════════════════════╪═════════════════════════╡
│ src/vulnlist/providers/rhel/parser.py             │                 21.591  │
├─────────────────────────────────────────────────┼─────────────────────────┤
│ src/vulnlist/providers/ubuntu/parser.py           │                 21.6144 │
├─────────────────────────────────────────────────┼─────────────────────────┤
│ tests/unit/providers/github/test_github.py      │                 35.3599 │
├─────────────────────────────────────────────────┼─────────────────────────┤
│ tests/unit/utils/test_oval_v2.py                │                 36.3388 │
├─────────────────────────────────────────────────┼─────────────────────────┤
│ src/vulnlist/providers/debian/parser.py           │                 37.3723 │
├─────────────────────────────────────────────────┼─────────────────────────┤
