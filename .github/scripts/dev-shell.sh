...
set +u
if [ -n "${DEV_VULNLIST_SHELL:-}" ]; then
    error "Already in a vulnlist development shell"
    exit 0
fi
set -u
...
title "Entering vulnlist development shell..."
...
step "Configuring with providers: $DEV_VULNLIST_PROVIDERS"
...
provider:
  ...
  configs:
EOF
for provider in $DEV_VULNLIST_PROVIDERS; do
...
step "Installing editable version of vulnlist"
...
export PATH=${DEV_VULNLIST_BIN_DIR}:$PATH
export DEV_VULNLIST_SHELL=true
...
unset DEV_VULNLIST_SHELL
unset DEV_VULNLIST_PROVIDERS

title "Exiting vulnlist development shell 👋"
